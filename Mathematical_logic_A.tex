...
\begin{comment}
3.12 一階述語論理の場合のLKの強い
形の完全性の証明
443
定理 56 強い形の完全性の証明:Bを言語Lの無 矛盾な理論とする．補題64より，言語Lを拡大 してL' とし，Bを拡大して B' として，無矛盾で witnessedな理論を得られる．B'はL' の理論であ る．これに補題65を適用して無矛盾かつ完全な 理論 3" を得る．B"はL' の理論であり，SCB" なので補題64より B" も witnessedな理論である． すると補題66より 3"はD = HUL であるような モデルを持つ． 補題64の証明: J( = {0}, 62, 63, ...})を新しい定
444
数記号の可算無限集合とする．KLN J = 0であ る．言語Lを拡大してKLY = KLUJとし，他の 記号は変えないものを言語 L' とする．KLは高々 可算無限集合なので，KLも可算無限集合とな る．言語 L' の論理式でちょうど一つの変数が自 由に一回以上出現し，他の変数は自由に出現しな
いようなものをEとする．ここでは VL, KL, FL, PLいずれもが高々可算無限集合 なのでE のような論理式は可算無限個存在する． それらをE1, E2, E3,... E Pres とし，それぞれ に自由に出現する変数を 11, 12,03, ... EVとす
445
る．ここで，n」を，bnがEに現れないような最 小のnとする．以下，nk-1 まで定められている として，bnがE1, E2, E3,.., E,に現れないよう な最小のnを n とする． F " 3x;E; → E[bn, /axi ] としてL' の閉論理式 Fiを定義する．= {F1, F2,Fe,... } とする． {Ex | i = 1, 2,3,.}は，ちょうど一つの変数のみ が自由に一回以上出現する L' の論理式を全て尽 くしているので，L' の理論 3' が8の拡大であれば B' が witnessedであるのは明らかである．
446
- 3x Eであるような1自由変数のEはあるiによ りE = Erで，tabn, とすればよい．則ち(2)が 成り立つ． BをLの無矛盾な理論とするとき，♂の元はすべ て閉論理式なのでBUSがL' の理論であることは 明らかである．したがって BUSが無矛盾である ことを示せば(1) を示したことになる． 30 3 とし，BA Bk-1 U {Fx} とおくと，各 Bk は理論 であり，BUS = U. BK となる． U.. Bk が矛盾し ていれば，ある有限のkで BK が矛盾しているの
447
で，すべてのkについて B, が無矛盾であること を示せばBUS が無矛盾であることがいえる．こ れを帰納法で示す．まず，Bo = Bなのでこれは 無矛盾である．次に，Bi-1が無矛盾であるとし
て B;が無矛盾であることを背理法で示す． Bi 上であると仮定する．帰納法の仮定より Bi-1トではないので，TC Bi-1であるような論 理式の有限列FがあってF,Tトが証明できるは ずである． 従って以下のような証明図を作ることができる． ここで S1, S2はシーケントを表すメタ変数で，大
448
きい証明図を分割表示するために導入した．
E;[bn/x; ] Er[bnr/xi]
= (--right) F -Er[On /vi], E, [on: /]
(weakening) S1 "" 12c;E; -E;[br /avi], E;[bn; //x; ]
S, "3x; E; → E;[br /z; }, T
449
Ei[z/gi] - Ei[z/gi]
ー(--left) - Ei [z/xi], Ei[z/ri] ト
(V-left) Fri - Ei, Eile/xi] F
=(ヨ-left) Vi-Eị, dai E, F.
ー (--left) 3x;E; E-Vxi-Ei
-- (weakening) 3riEx - Vei-Ei, Eilon; /vi]
3riEi -VaiE^ -Ei[bnt/xi], Ei[bnt/gi]
ト -Vei Ei - Ei[bn; /xi], ヨxi Ei → Ei[bnj/xi] (cut)
-Vai Ei ^ - Ei[on; /ii]
S1
- (^-right) (→-right) ．
この証明図に現れない変数記号をxとし，証明図 のbn, を全て公に置換する． n; の与え方から6n, はTに現れないため，TE -VI-E; -Erfac/xi] の証明図となる．これを一般に以下が証明図の一
450
部となることと併せる．
EFE ΓΕΕΛΕΙ ΕΛΕΕΕ
- (^-left1)
(cut) THE TF -vr;E; とTF -E[x/x; ] を証明できる．
TE-E; [x /ti] (H-right)
TE Vxi-Ei となるので，TE --E; と併せて Bi-1が無矛 盾であることに反する．
451
リンデンバウムの補題(補題65)の証明: まず準備として，a C Pret が無矛盾かつa より 真に大きいPres の部分集合が全て矛盾している 時，aは極大無矛盾であると言う．これは命題論 理の場合と同様の定義である． (極大)無矛盾な閉論理式の集合に対し，命題論理 の場合の補題20から補題23までと同様の補題が 成立する．但しGをa，ProをPrefに置き替え たものである．証明はほとんど並行して記述でき る．但し補題23の証明の後に書いてある，元の 論理式Gを極大化したG'の決定可能性について
452
は成り立たない．証明中のG - X が決定可能で はないためである． 与えられた無矛盾な理論 a に対し補題23(の Pres 版)を適用すると極大無矛盾な理論 a' を得
る．補題21(同前)によりa'は完全である． ロ 補題66の証明: BC Pret なのでD = HULとお いて〈D, I> BとなるIを与えられればよい．
I(c) ", I(f)(t1,.., ta) " f(t1,.., th).
(99) (100)
453
として定数記号と函数の意味を定める(エルブラ ン解釈)．ここにI(C)のcはKの元としてのcで あり，右辺のcは HULの元としてのcである．同 様にI(f)の f は F の元であり，t1,..., tと右辺の f(t1,.., th)はHUL の元である．また，
I(p)(t1,.., th) =TH BF p(t1,...,ts)
(t1,...,tr E HUL). (101)
としてI(p)を定める．すると 〈D, I〉 = p(t1, ...,ts) iff B pt1,...,ts)となる．
454
以下では，閉論理式E' Prefについて 〈D, I> E' iff BF E'であることをE'に現れる AV → - V3の数の合計 no(E')による帰納法によ り証明する．これが証明できれば，任意のFEB について，BFFであることから〈D, I〉 F と なり，《D, I〉がBのモデルになっていることが示 される． 1. no(E') = 0，すなわちE' EAALのとき E = p(t1,.... th)の形である．t1,.... tyは閉項な ので，先に見た通りI(p)(t1,..., th) iff BE pt1,.... tw)であり，帰納法のベースは満たさ
455
れている． 2a. E' = EAFの形の時，EもFも閉論理式で ある．no(E) < no(E')，no(F) < no(E') となる ので，帰納法の仮定により 〈D, I> Eiff BF E であり，《D, I> Fiff BF F である． ここで〈D, I〉 EAFを仮定すると〈D, I〉 E かつ(D, I〉 — Fなので，帰納法の仮定より BEEかつBFFである．したがってT.CBで あるような有限列TがあってTEEがLKで証明 でき，△, C B, AFFについて同様のことが成 り立つ．すると以下のようにしてT, AF EAF
456
が証明できるため，BE EAFとなる．
TEE (weakening, AFF con TAFE exchange) TALE (Weakening)
Γ, ΔΕΕΛF
- (1-right) 逆にTsa C B, TF EAFであれば，以下のよう にしてTFEとなり，同様にしてTIFとなる．
EEE
- (^-left2) ΓΕ ΕΛF ΕΛFEE
THE
457
したがって BEEかつ BFFである．すると帰納 法の仮定より 〈D,IF E かつ(D,I) F とな り，〈D,I) - EAFとなる． E'がEVF,E → F,-Eの形のときも同様の議論 により〈D, I> > E' iff BF E'となる．なお EAFの形の時には使わなかった事実として以下 が成り立つことに注意すること:Bは無矛盾かつ 完全な理論なので，B4 Eiff BF -Eである． 2b. E' = VIEの形の時，Eが閉論理式の時と自 由変数としてのみが出現している時に分けられ る(この部分は，2つに分ずに以下の「後者の場
458
合」のみを考えて，後でEが閉の場合にはt= としてもよい)．
■前者の場合 no(E) < no(E')なので，帰納法の仮定より 〈D, I> > Eiff BF Eが成り立つ．また，その場 合には〈D, I> VrEiff 〈D, I〉 E であり，以 下の証明図からBF Eiff BF VEなので (D, I) E VxE iff B E VxE 43.
459
TEE
TEHF (V-right)
EFE_ (H-left)
...
(cut)
■後者の場合
Eにxの自由な出現が1回以上ある場合である．
((D, I) E VxE + BE VxE) BH VxEYE らば〈D, I〉 - VuEを示す．Bは完全なので
460
BH VxE2532ZD BE VxET. 23bIs CB, TE VxE27 Tubo
Ez/x] E E[z/x]
- (--right) + E[z/x], -E[z/x]
- (3-right) E E[z/x], 3x-E EVxE, 3x-E
= (V-right, *28)
"
-(--left) TE VxE VxE F 3x-E
(cut) TE 3x-E
461
上の証明図より BF 3x-Eが証明できる．Bは witnessedなので，基礎項tがあって BE -E[t/a] である．補題の仮定よりBは無矛盾なので， By E[t/]である．すると， no(E[t/x]) = no(E) < no(VxE) = no(E') TDS ることと，tが基礎項でE[t/ar] Pref なので帰 納法の仮定を使って〈D, I> LE[t/x] となる．同 じ理由でどんなに対しても〈D, I> E[t/ac] iff 〈D,I), vE[t/ar]である故，どんなルに対して
*28 zは下の行に現れない新しい変数．
462
も〈D,I), v = E[t/a] となる．D = HULなの で，任意の基礎項SE D，(99) ・ (100)等で定義 したI，任意のに対し〈D,I),v[s/x] = Eiff 〈D,I),v E[s/] が成り立つため，どんなに 対しても〈D, I),v[t/ac] ¥ E となる．上の定義よ り，〈D,I),v = VuEiff 全てのdEDについて 〈D, I),v[d/x] Eだからどんなに対しても (D, I), v E VxE76 3. VxE E Pre Ents vrt LT (D, I) E VxE iff (D, I), v E VxE なので結局〈D, I> VIEが言える． (B+VxE = (D,I) E VxE) ČR, BEVE)
463
{ $k (D, I) EVET . (D, I) E VxE iff (D, I), V = VxE iff A TOLED = HULKXit L (D, I), v[t/x] E E iff & Tote DKML (D, I), v E Est/x) iff È TOE E Drittu <D,I) > E[t/ac] 故，全てのtE D = HULに対
し〈D, I> > E[t/a] を示せばよい． まず，BF VEより，TCBである「と TF VIE の証明があるので，任意のtE Dに対し て以下の証明図のように BE E[t/z]である． E[t/z]は閉論理式なので，帰納法の仮定より 〈D, I> E[t/a]が成り立つ．
464
E[t/ar] - E[t/ar] |
- (V-left) TEVE VxEF E[t/x]
!? (cut) TE E[t/x] 2c. E' = 3CEの形の時については省略する．2b の場合と似た道筋で証明できる(Vの場合と逆に， BE3xEDP 5 (D,I) = 3xEXTR
witnessed という性質を用いる)．
465
\end{comment}
\end{document}
